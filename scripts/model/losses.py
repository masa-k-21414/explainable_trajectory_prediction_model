...
def gan_g_loss(scores_fake, loss_type='bce'):
    """
    Input:
...
def gan_d_loss(scores_real, scores_fake, loss_type='bce'):
    """
    Input:
    ...
    if loss_type == 'bce':
        ...
        loss_real = - torch.min(scores_real - 1, torch.zeros_like(scores_real)).mean()
        loss_fake = - torch.min(-scores_fake - 1, torch.zeros_like(scores_fake)).mean()
...
def a_l2_loss(at_gt, at_pr, random=0, mode='average'):
    """
    Input:
    - 
    Output:
